...
public class PDFStream extends PDFDictionary implements PDFConstants {
    ...
    public void colorSpace(float[] cc)
    throws IOException {
    	String colorspace = "";
    	for(int i = 0; i < cc.length; i++)
    	{
    		colorspace += PDFUtil.fixedPrecision(cc[i]) + " ";
    	}
    	println("/Cs8 cs " + colorspace + "scn");    	
    }

	public void colorSpaceStroke(float[] cc)
	    throws IOException {
    	String colorspace = "";
    	for(int i = 0; i < cc.length; i++)
    	{
	...
	public void colorSpace(String colorSpace, float[] cc)
	    throws IOException {
	    		    	
	    	println("/" + colorSpace.replace(" ","#20") + " cs 1 " + "scn");    	
	}

	public void colorSpaceStroke(String colorSpace, float[] cc)
	    throws IOException {
    	String color = "";    	
    	println("/" + colorSpace.replace(" ", "#20") + " CS 1 " + color + "SCN");   
	}

    public void colorSpace(String colorSpace, float tintValue)
            throws IOException {

        println("/" + colorSpace.replace(" ","#20") + " cs " + String.format("%f ", tintValue) + "scn");
    }

    public void colorSpaceStroke(String colorSpace, float tintValue)
            throws IOException {
        println("/" + colorSpace.replace(" ", "#20") + " CS " + String.format("%f ", tintValue) + "SCN");
    }
    //
