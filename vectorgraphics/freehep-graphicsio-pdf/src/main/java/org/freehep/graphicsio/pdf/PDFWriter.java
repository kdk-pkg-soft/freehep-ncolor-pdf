...
public class PDFWriter extends PDF implements PDFConstants {
    ...
    public PDFObject openObject(String name) throws IOException {
        ...
        if (open != null)
            System.err
                    .println("PDFWriter error: 22 '" + open + "' was not closed");
        open = "PDFObject: " + name;
    ...
    public PDFDeviceNColor openDeviceNColor(String name, Object[] colorObjects)
            throws IOException {
        PDFObject object = openObject(name);                        
        PDFDeviceNColor deviceN = object.openDeviceNColor(this, colorObjects);
        return deviceN;
    }
    
	public void close(PDFDeviceNColor ncolor) throws IOException {
		ncolor.close(true);		
		open = null;
	}
	
    public PDFNColor openNColorFunction(String name, int colorSize) throws IOException {    	
        return openNColorFunction(name, null, colorSize);
    }

    public PDFNColor openNColorFunction(String name, String[] encode, int colorSize)
            throws IOException {
        PDFObject object = openObject(name);
        PDFNColor ncolor = object.openNColorFunction(name, encode, colorSize);   
        ncolor.entry("Length", ref(name + lengthSuffix));
        return ncolor;
    }
    
	public void close(PDFNColor ncolor) throws IOException {
		ncolor.close();
		open = null;
		object(ncolor.getName() + lengthSuffix, ncolor.getLength());		
	}

    public PDFSeparation openSeparation(String spotName)
	    throws IOException {
		PDFObject object = openObject(spotName);                        
		PDFSeparation deviceN = object.openPDFSeparation(this, spotName);
		return deviceN;
	}
	
	public void close(PDFSeparation ncolor) throws IOException {
		ncolor.close(true);		
		open = null;
	}

	public PDFSeparationFunction openSeparationFunction(String name, String[] encode)
       throws IOException {
	   PDFObject object = openObject(name);
	   PDFSeparationFunction ncolor = object.openSeparationFunction(name, encode);   
	   ncolor.entry("Length", ref(name + lengthSuffix));
	   return ncolor;
	}
	
	public void close(PDFSeparationFunction ncolor) throws IOException {
		ncolor.close();
		open = null;
		object(ncolor.getName() + lengthSuffix, ncolor.getLength());		
	}
