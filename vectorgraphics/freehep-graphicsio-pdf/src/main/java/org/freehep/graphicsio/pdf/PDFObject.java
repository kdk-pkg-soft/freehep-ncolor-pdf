...
public class PDFObject implements PDFConstants {
    ...
    PDFDictionary openDictionary() throws IOException {
        ...
        if (open != null)
            System.err
                    .println("PDFWriter error: 4'" + open + "' was not closed");
        open = "PDFDictionary";
    ...
    PDFStream openStream(String name, String[] encode) throws IOException {
        ...
        if (open != null)
            System.err
                    .println("PDFWriter error: 5'" + open + "' was not closed");
        open = "PDFStream";
    ...
    PDFNColor openNColorFunction(String name, String[] encode, int colorSize) throws IOException {
        if (!ok)
            System.err.println("PDFWriter error: 'PDFStream' was closed");
        if (open != null)
            System.err
                    .println("PDFWriter error: 6'" + open + "' was not closed");
        open = "PDFNColor";
        PDFNColor ncolor = new PDFNColor(pdf, out, name, this, encode, colorSize);
        return ncolor;
    }
    
    void close(PDFNColor ncolor) throws IOException
    {
    	ncolor.close();
    	open = null;
    }
    
    PDFSeparationFunction openSeparationFunction(String name, String[] encode) throws IOException {
        if (!ok)
            System.err.println("PDFWriter error: 'PDFStream' was closed");
        if (open != null)
            System.err
                    .println("PDFWriter error: 6'" + open + "' was not closed");
        open = "PDFNColor";
        PDFSeparationFunction ncolor = new PDFSeparationFunction(pdf, out, name, this, encode);
        return ncolor;
    }
    
    void close(PDFSeparationFunction ncolor) throws IOException
    {
    	ncolor.close();
    	open = null;
    }
    
    PDFDeviceNColor openDeviceNColor(PDF pdf, Object[] colorObjects) throws IOException {
        if (!ok)
            System.err.println("PDFWriter error: 'PDFStream' was closed");
        if (open != null)
            System.err
                    .println("PDFWriter error: 7'" + open + "' was not closed");
        open = "PDFDeviceNColor";
        PDFDeviceNColor ncolor = new PDFDeviceNColor(pdf, out, colorObjects, this);
        return ncolor;
    }
    
    void close(PDFDeviceNColor deviceNcolor) throws IOException
    {
    	deviceNcolor.close();
    	open = null;
    }

    PDFSeparation openPDFSeparation(PDF pdf, String spotName) throws IOException {
        if (!ok)
            System.err.println("PDFWriter error: 'PDFStream' was closed");
        if (open != null)
            System.err
                    .println("PDFWriter error: 7'" + open + "' was not closed");
        open = "PDFDeviceNColor";
        PDFSeparation separationColor = new PDFSeparation(pdf, out, spotName, this);
        return separationColor;
    }
    
    void close(PDFSeparation separationColor) throws IOException
    {
    	separationColor.close();
    	open = null;
    }
    ...
    PDFDocInfo openDocInfo(PDF pdf) throws IOException {
        ...
        if (open != null)
            System.err
                    .println("PDFWriter error: 8'" + open + "' was not closed");
        open = "PDFDocInfo";
    ...
    PDFCatalog openCatalog(PDF pdf, PDFRef pageTree) throws IOException {
        ...
        if (open != null)
            System.err
                    .println("PDFWriter error: 9'" + open + "' was not closed");
        open = "PDFCatalog";
