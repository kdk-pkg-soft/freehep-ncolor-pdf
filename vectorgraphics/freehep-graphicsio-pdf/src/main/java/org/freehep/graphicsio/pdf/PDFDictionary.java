...
public class PDFDictionary implements PDFConstants {
    ...
    PDFDictionary(PDF pdf, PDFByteWriter writer, PDFObject parent, boolean objectInDisguised)
    	throws IOException {
		this.pdf = pdf;
		object = parent;
		out = writer;
    ...
    void close(boolean objectInDisguised) throws IOException {
        if (open != null)
            System.err
                    .println("PDFWriter error: 1'" + open + "' was not closed");
        out.outdent();
        out.println("]");
        if (object != null)
            object.close();
    ...
    void close() throws IOException {
        if (open != null)
            System.err
                    .println("PDFWriter error: 2'" + open + "' was not closed");
        out.outdent();
        out.println(">>");
        if (object != null)
            object.close();
    ...
    public void entry(String key, String string,boolean original) throws IOException {
        if (!ok)
            System.err.println("PDFWriter error: 'PDFDictionary' was closed");
        if (original) {
   ...
   public void entry(String key, PDFName name) throws IOException {
    ...
    public PDFDictionary openDictionary(String name) throws IOException {
        ...
        if (open != null)
            System.err
                    .println("PDFWriter error: 3'" + open + "' was not closed");
